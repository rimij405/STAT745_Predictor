...
plot(VIF.lm, xlab = "Predictor", ylab = "VIF", main = "VIFs of lm")
```
...
par(mfrow=c(2,2))
plot(Model$lm.clean)
plot(Model$lm.clean$fitted.values, hatvalues(Model$lm.clean))
...
par(mfrow=c(2,2))
plot(Model$lm.quad)
plot(Model$lm.quad$fitted.values, hatvalues(Model$lm.quad))
```
...
# PCA model.
form <- as.formula(paste0("Y ~ ", paste0("PC", 1:70, collapse = "+")))
Model$lm.pca <- lm(form, data = Train$pca)
...
par(mfrow=c(2,2))
plot(Model$lm.pca)
plot(Model$lm.pca$fitted.values, hatvalues(Model$lm.pca))
```

```{r subsets1}
Model$lm.forward <- regsubsets(Y ~ ., data = Train$clean, nvmax = 140, method = "forward")
...
Train$clean.mat = model.matrix(Y ~ ., data = Train$clean)
val_errors = rep(NA, 140)
for(i in 1:140) {
  coefs = coef(Model$lm.forward, id = i)
  preds = Train$clean.mat[,names(coefs)] %*% coefs
  val_errors[i] = mean((Train$clean$Y - preds)**2)
}

# Select best model with smallest error.
plot(val_errors)
points(which.min(val_errors), val_errors[which.min(val_errors)][1], col = "red", cex = 2, pch = 20)
print(coef(Model$lm.forward, id = which.min(val_errors)))
train.mse <- val_errors[which.min(val_errors)][1]
writeLines(str_glue("Training MSE (Model$lm.forward): {round(train.mse, 2)}"))
```

```{r subsets2}
Model$quad.forward <- regsubsets(Y ~ ., data = Train$quadratic, nvmax = 200, method = "forward")
Model$quad.forward.summary <- summary(Model$quad.forward)
plot(Model$quad.forward.summary$adjr2, apply(Model$quad.forward.summary$which, 1, sum), ylab = "Number of Predictors")
plot(Model$quad.forward)

# Select best by BIC.
Model$quad.forward.BIC <- Model$quad.forward.summary$which[which.min(Model$quad.forward.summary$bic),]
print(Model$quad.forward.BIC)
print(sprintf("There are %d predictors in the model selected by %s.", sum(Model$quad.forward.BIC), "BIC"))

print(coef(Model$quad.forward, 25))

# Calculate MSE errors.
Train$quad.mat = model.matrix(Y ~ ., data = Train$quadratic)
val_errors = rep(NA, 140)
for(i in 1:140) {
  coefs = coef(Model$quad.forward, id = i)
  preds = Train$quad.mat[,names(coefs)] %*% coefs
  val_errors[i] = mean((Train$quadratic$Y - preds)**2)
}

# Select best model with smallest error.
plot(val_errors)
points(which.min(val_errors), val_errors[which.min(val_errors)][1], col = "red", cex = 2, pch = 20)
print(coef(Model$quad.forward, id = which.min(val_errors)))
train.mse <- val_errors[which.min(val_errors)][1]
writeLines(str_glue("Training MSE (Model$quad.forward): {round(train.mse, 2)}"))
```
...
```
```{r extents-analysis2}
mat <- apply(Train$clean**2, 2, range)
p <- ncol(Train$clean**2)
matplot(2:p, t(mat)[-1,], type="l", main = "Maximum and Minima")
```
